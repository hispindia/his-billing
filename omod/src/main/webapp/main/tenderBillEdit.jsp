...
<div id="tabs">
<ul>
	<li><a href="#fragment"  style="color: white"><span>List Tenders</span></a></li>
</ul>
<div id="fragment">
	<c:forEach items="${listTender}" var="tender">
		<div id="tender_${tender.tenderId}"  class="udiv boxNormal" onclick="addToBill('${tender.tenderId}', '${tender.price}', '${tender.name}')">
			...
			<div id="detail_${tender.tenderId}" class="detailDiv">
				<ul style=" list-style: none; padding: 0; margin: 0;">
					...
					<li><b>Opening date:</b> <openmrs:formatDate date="${tender.openingDate }" type="textbox" /></li>
					<li><b>Closing date:</b> <openmrs:formatDate date="${tender.closingDate }" type="textbox" /></li>
				</ul>
			</div>
		</div>
	</c:forEach>
...
<div id="billDiv">
<form method="POST" id="billForm" action="editTenderBill.form" onsubmit="return false">
	<input type="hidden" value="" name="action" id="action"/>
	<div> 
		<input type="button" onclick="validateForm()" id="subm" name="subm" value="<spring:message code='billing.bill.save'/>" />
		<input type="button"  id="voi" onclick="voidBill()"  value="<spring:message code='billing.bill.void'/>" />
		<input type="button" value="<spring:message code='general.cancel'/>" onclick="javascript:window.location.href='tenderBill.list?companyId=${companyId}'" />
		<input type="button" id="toogleBillBtn" value="-" onclick="toogleBill(this);" class="min" style="float: right" />
	</div>
	...
	<input type="hidden" name="tenderBillId" value="${tenderBill.tenderBillId}"> 
...
<c:forEach items="${tenderBill.billItems}" var="bill">
	...
	<c:set var="serviceName" value="${fn:replace(bill.name,search,replace)}"/>	
	<script type="text/javascript">
